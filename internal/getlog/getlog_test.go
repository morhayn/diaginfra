...
func TestGetLogs(t *testing.T) {
	...
	t.Run("test fpko", func(t *testing.T) {
		...
		assert.Equal(t, res, "sudo tail -n 1000 /d01/tomcat/tomcat1/logs/wap-test.log")
	})
	t.Run("test scuo", func(t *testing.T) {
		g.Service = "tomcat"
		logs := map[string]string{"tomcat": "/var/log/tomcat8/"}
		res := g.GetLogs(logs, mock)
		assert.Equal(t, res, "sudo tail -n 1000 /var/log/tomcat8/wap-test.log")
	})
